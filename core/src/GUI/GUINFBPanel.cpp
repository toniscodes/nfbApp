...
#include "../../include/GUI/GUINFBPanel.h"
...
void GUINFBPanel::setStartUpValues(int xP, int yP, int panelItemSizeYP, int widthP, int heightP, int scrollBarWidthP) {
    x=xP;
    y=yP;
    width=widthP;
...
// Note: The height isn't in pixels. It's the multiply of the panelItemSizeY
GUINFBPanel::GUINFBPanel(int xP, int yP, int panelItemSizeYP, int widthP, int heightP, int scrollBarWidthP, int zIndexP, string toolTipText) {
    setZ(zIndexP);
    //GUINFBPanel(xP, yP, panelItemSizeYP, widthP, heightP, scrollBarWidthP);
...
// Note: The height isn't in pixels. It's the multiply of the panelItemSizeY
GUINFBPanel::GUINFBPanel(int xP, int yP, int panelItemSizeYP, int widthP, int heightP, int scrollBarWidthP, string toolTipText)
{
    setStartUpValues(xP, yP, panelItemSizeYP, widthP, heightP, scrollBarWidthP);
    GUIElement::setToolTipArea(xP,yP,width,height,toolTipText);
    originalToolTipTxt=toolTipText;
...
string GUINFBPanel::getSelectionItemId() {
     return pItems[selectionIndex].getItemId();
}

void GUINFBPanel::addItem(string itemText, string itemId, ALLEGRO_COLOR itemColor) {
...
bool GUINFBPanel::mouseIsInsideScrollPanel() {
...
bool GUINFBPanel::mouseIsInsideScrollPanelTextPart() {
...
bool GUINFBPanel::mouseIsInsideScrollPanelSlider() {
...
bool GUINFBPanel::mouseIsInsideScrollPanelSliderMiddleButton() {
    if (guiMouse.getMouseX()>x+width-scrollBarWidth && guiMouse.getMouseX()<x+width && guiMouse.getMouseY()>y && guiMouse.getMouseY()<y+height) {
...
void GUINFBPanel::adjustVisibility(int direction) {
...
void GUINFBPanel::lockHighlightItem() {
    lockedHighLightItem = selectionIndex;
}

void GUINFBPanel::clearItems() {
    pItems.clear();
}

void GUINFBPanel::limitSelectionIndexBoundaries() {
    if (selectionIndex<0)
        selectionIndex=0;
...
void GUINFBPanel::loopSelectionIndexInBoundaries() {
    selectionIndex++;
    if (selectionIndex<0)
        selectionIndex=pItems.size()-1;
    if (selectionIndex>pItems.size()-1)
...
void GUINFBPanel::loopLockedHighlightIndexInBoundaries() {
    lockedHighLightItem++;
...
void GUINFBPanel::highlightNextItem() {
    loopSelectionIndexInBoundaries();
}

void GUINFBPanel::GUINFBPanel::selectNextItem() {
...
void GUINFBPanel::selectItemById(string searchId) {
    ...
    al_set_clipping_rectangle(x, y, width-scrollBarWidth, height); // Notice that this is made different later for that scrollbar
        ...
        for(size_t i = startingIndex; i < endingIndex; ++i)
        {
           ...
           if (libraryFont) {
            ...
            ALLEGRO_COLOR colorMaximum =
                      al_map_rgba_f(0.62f,
                                    0.42f,
                                    0.42f,
                                    0.42f);

            ALLEGRO_COLOR colorMinimum =
                      al_map_rgba_f(0.32f,
                                    0.42f,
                                    0.32f,
                                    0.42f);

              // Draw the graph for NEUROFEEDBACK BAR:
              ...
              // Draw the maximum:
              if (bwMeter.thisChannelSlowBarMaximum((strToInt(pItems[i].getItemId())))>0.0f) {
                  float barPos = ((width-scrollBarWidth)*(2.0f/3.0f))*(bwMeter.thisChannelSlowBarMaximum((strToInt(pItems[i].getItemId()))));
                  al_draw_filled_rectangle(posX+width/3+(barPos), posY, posX+width/3+(barPos)+1, posY+panelItemSizeY, colorMaximum);
              }

              // Draw the minimum:
              if (bwMeter.thisChannelSlowBarMinimum((strToInt(pItems[i].getItemId())))>0.0f) {
                  float barPos = ((width-scrollBarWidth)*(2.0f/3.0f))*(bwMeter.thisChannelSlowBarMinimum((strToInt(pItems[i].getItemId()))));
                  al_draw_filled_rectangle(posX+width/3+(barPos), posY, posX+width/3+(barPos)+1, posY+panelItemSizeY, colorMinimum);
              }

              // Draw the avarage:
              if (bwMeter.thisChannelSlowBarAvg((strToInt(pItems[i].getItemId())))>0.0f) {
                  float barPos = ((width-scrollBarWidth)*(2.0f/3.0f))*(bwMeter.thisChannelSlowBarAvg((strToInt(pItems[i].getItemId()))));
                  al_draw_filled_rectangle(posX+width/3+(barPos), posY, posX+width/3+(barPos)+1, posY+panelItemSizeY, colorMinimum);
              }
