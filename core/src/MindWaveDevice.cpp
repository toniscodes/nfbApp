...
void MindWaveDevice::init() {
    ...
    for (int i=0;i<TG_AMOUNT_OF_CHANNELS;i++) {
        NeuroSkyScaleFactor[i]=1.0d; // Please don't change this to prevent scaling of attention and meditation etc.
        lastValue[i]=0.0d;
    }
    ...
    do {
        ...
        if( packetsRead == 1 ) {
            ...
            if( TG_GetValueStatus(connectionId, TG_DATA_RAW) != 0 ) {
                ...
                // Dummy recording of EEG-raw data.
                bwManager.updateRawValues(0,lastValue[TG_TMP_RAW]);
