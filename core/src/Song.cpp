...
/*float Song::getAvgFlowValueZeroToHundred() const {
    ...
    return songBwInfo.channelInfo[TG_GRAPH_ATT].getAvarageValue()*ATT_FACTOR - songBwInfo.channelInfo[TG_GRAPH_MED].getAvarageValue()*MED_FACTOR; // -0.75 - 1. neg is too lazy, 1 is too anxious
}*/
...
{
    return "Avarage value of " + getGraphTheme(chanIx).text + " is " + floatToStr(songBwInfo.channelInfo[chanIx].getAvarageValue()*getGraphTheme(chanIx).scaleFactor) + "\n";
}
...
{
    return floatToStr(getChannelAvg(chanIx));
}

// Get channel avarage of asked index.
float Song::getChannelAvg(int chanIx) const
{
    return songBwInfo.channelInfo[chanIx].getAvarageValue()*getGraphTheme(chanIx).scaleFactor;
}
...
{
    string returnStr = "";
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_DELTA);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_THETA);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_ALPHA);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_BETA);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_GAMMA);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_AVG);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_ATT);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_MED);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_FLOW);
    returnStr += getStrChannelAvgAndDescription(TG_GRAPH_CALMNESS);
    return returnStr;
}
...
bool Song::isBwInfoUsable() const
{
    ...
    if (songBwInfo.isReady())
    {
        ...
        for (int i=0; i<TG_AMOUNT_OF_CHANNELS; i++)
            anyValueAdded += songBwInfo.channelInfo[i].getAvarageValue();

        if (anyValueAdded>10000)
            return true;
...
ALLEGRO_COLOR Song::getTextColor() const
{
    ...
    ALLEGRO_COLOR theColor = al_map_rgb_f(0.0, 0.3, 0.3); // This is init color If no bw data is available.

    if (songBwInfo.isReady())
    {

        theColor = al_map_rgb_f(0.0, 0.0, 1.0);
        float minValue = 10;


        theColor = al_map_rgb_f(0.2, 0.2, 0.2); // Not added anything much but just a little?
        if (isBwInfoUsable())
            theColor = al_map_rgb_f(0.3, 0.3, 0.3); // Something added! whyy this is cool

        // These colors are cumulated a bit to make it look something.

        float amountOfActivity = musicLibrary.getRelativeValueInAllSongsOfChannel(bwMeter.PROFILE_CHANNEL_1,getChannelAvg(bwMeter.PROFILE_CHANNEL_1));

        //playerLog("activity actual: " + floatToStr(getChannelAvg(bwMeter.PROFILE_CHANNEL_1)));
        //playerLog("activity relative: " + floatToStr(amountOfActivity));
        /*    amountOfActivity=amountOfActivity*100.0f;
            amountOfActivity*=amountOfActivity;
            amountOfActivity*=0.001f;
            if (amountOfActivity>1.0f)
                amountOfActivity = 1.0f;*/

        float amountOfLiking = musicLibrary.getRelativeValueInAllSongsOfChannel(bwMeter.PROFILE_CHANNEL_2,getChannelAvg(bwMeter.PROFILE_CHANNEL_2));

        //playerLog("liking: " + floatToStr(getChannelAvg(bwMeter.PROFILE_CHANNEL_2)));
        //playerLog("liking relative: \n\n" + floatToStr(amountOfLiking));
        /*amountOfLiking=amountOfLiking*100.0f;
        amountOfLiking*=amountOfLiking;
        amountOfLiking*=0.001f;
        if (amountOfLiking>1.0f)
            amountOfLiking = 1.0f;*/

        theColor = getMapColor(amountOfActivity,amountOfLiking);

        //theColor = al_map_rgb_f(amountOfActivity, amountOfLiking, 0.25f);

        //theColor = al_map_rgb_f(amountOfActivity, 0.0f, 0.5f);

    } // Was the pre-info created to intialize color for song?
...
float Song::getMapDistanceFromTimeLinePointAutoAxis(float v1, float v2, int timeP)
{
    ...
    if (djModeUseXAxis.isChecked())
        // We don't have the graph anymore there.. So just use the smoothened compressed data :)
        //amountOfActivity = musicLibrary.getRelativeValueInAllSongsOfChannel(bwMeter.PROFILE_CHANNEL_1,songBwInfo.channelInfo[bwMeter.PROFILE_CHANNEL_1].getBWGraph().getScaledValue(timeP));
    ...
    //amountOfLiking   = musicLibrary.getRelativeValueInAllSongsOfChannel(bwMeter.PROFILE_CHANNEL_2,songBwInfo.channelInfo[bwMeter.PROFILE_CHANNEL_2].getBWGraph().getScaledValue(timeP));
